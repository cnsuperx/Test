...
void LayerLogin::callNull1( CCObject* ted )
{
    
}
void LayerLogin::callNull2( CCObject* ted )
{
...
//    CCLabelTTF *version=CCLabelTTF::create("V1.2", "hycc.ttf", 18);
...
//    if ((nameString->length()!=0)&&(passwordString->length()!=0)){
    if(strcmp(editBoxUsername->getText(), "")&&strcmp(editBoxPassword->getText(), ""))
    {
        ...
        if(uIllegal>0||pIllegal>0)
        {
			...
			CustomPop::show("The user name or password contains illegal characters");
        }
        else
        {
			// 添加一个Loading的Layer层，并设置触摸吞噬
            load=Loading::create();
            addChild(load,9999);
    ...
    else
    {
		...
		CustomPop::show("The user name or password cannot be empty");
    }
}
...
void LayerLogin::sendPersonalData(float delta){
...
void LayerLogin::receiveCityData(float delta)
{
...
void LayerLogin::receivePersonalData(float delta){
