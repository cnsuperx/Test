...
void xuanren::FirstPerson( CCObject* det )
{
	if(flag!=1){
...
void xuanren::SecondPerson( CCObject* det )
{
	if(flag!=2){
...
void xuanren::ThridPerson( CCObject* det )
{
	if(flag!=3){
...
void xuanren::EnterName( CCObject* det )
{
...
void xuanren::replacScene( CCObject* det )
{
	...
	else
	{
		...
		CustomPop::show("name can't empty");
		//load->removeFromParent();
	}
}
void xuanren::receiveRoleInfo( float det )
{
		...
		if(datastr){
			...
			if (read.parse(datastr, root)) {
				...
				if(result){
					...
					CustomPop::show("User name already has");
					load->removeFromParent();
				}
			}
		}else{
...
void xuanren::receiveCityData( float det )
{
...
void xuanren::receiveHomeInfo( float det )
{
