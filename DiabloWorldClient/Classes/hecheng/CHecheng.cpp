...
void CHecheng::openbag( CCObject* )
{
    clearData();
...
//发送请求
void CHecheng::fasong( CCObject* )
{
    
    if(tmpitemdata == NULL)
    {
        //CustomPop::show("请选择合成材料");
		CustomPop::show("Please select a synthetic material");
        return;
    }
...
void CHecheng::jsondata(char * data)
{
    ...
    if(read.parse(data, value))
    {
		...
		if(ischenggong == true)
		{
			...
			//CustomPop::show("合成成功！");
			CustomPop::show("Synthesis of success");
